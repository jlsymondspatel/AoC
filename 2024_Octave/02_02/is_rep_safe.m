function rep_rep = is_rep_safe (rep)
    ...
    rep_rep.mask_max_diff = rep_rep.diffs_abs > 4;

    # make mask for the sign of each difference
    rep_rep.mask_diff_sign = sign (rep_rep.diffs);
    ...
    if (sum (rep_rep.mask_diff_sign) >= 0) # if the majority of levs are increasing
        # count only those diffs that are decreasing
        rep_rep.viols(3) = sum (rep_rep.mask_diff_sign < 0);
